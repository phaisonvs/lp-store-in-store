  ...
  if (elements.length > 0) {
    ...
    setTimeout(() => {
      elements.forEach((el) => {
        ...
        // Tornar visível via opacidade ANTES de animar (necessário pois removemos visibility:hidden)
        // el.style.opacity = '1'; // Teste: talvez não precise, Animate.css deve cuidar
        // el.style.visibility = 'visible'; // REMOVIDO

        // Adicionar classes Animate.css
        const animationClass = "animate__zoomIn"; // Manter zoomIn
        ...
        // Opcional: Remover classes após a animação e adicionar classe final
        el.addEventListener(
          "animationend",
          () => {
            console.log(
            ...
            el.classList.add("animation-finished"); // Adiciona classe para manter opacidade 1
          },
          { once: true }
        );
      });
