  ...
  if (elementsToAnimate.length > 0 && provaSocialContainer) {
    ...
    const onAnimationEnd = () => {
      ...
      if (animationsFinishedCount === totalAnimations) {
        console.log("Animações iniciais concluídas. Exibindo prova social.");
    ...
    if (provaSocialContainer && elementsToAnimate.length === 0) {
      console.log(
        "Elementos de animação não encontrados, exibindo prova social diretamente."
      );
      // Exibir imediatamente ou após um pequeno delay, já que não há animações para esperar
      setTimeout(() => {
        provaSocialContainer.classList.add("prova-social-visivel");
      }, 100);
    }
