...
/**
 * @description Inicializa o efeito de typing para o texto principal usando clip-path
 */
function setupTypingEffect() {
  ...
  if (headingElement) {
    ...
    // Pega o texto original e calcula caracteres
    const textoOriginal = headingElement.textContent
      .trim()
      .replace(/\s+/g, " ");
    const numeroCaracteres = textoOriginal.length;

    // Atualiza a variável CSS com o número de caracteres para a animação steps()
    headingElement.style.setProperty("--char-count", numeroCaracteres);

    // Garante que o texto original esteja presente para o clip-path funcionar
    // (O texto é visualmente escondido pelo clip-path inicial)
    headingElement.textContent = textoOriginal;

    // Aguarda a animação dos SVGs terminar
    setTimeout(() => {
      // Adiciona a classe para iniciar a animação de clip-path e cursor
      headingElement.classList.add("typing-active");

      // Monitora o fim da animação no elemento principal
      const handleAnimationEnd = (e) => {
        // Verifica se a animação de typing (clip-path) terminou
        if (e.animationName === "typing-clip") {
          console.log("Animação de typing (clip-path) concluída");
