...
function triggerImmediateEntranceAnimation() {
  const elementsToAnimate = [
    document.querySelector(".svg-suf-desk"),
    document.querySelector(".svg-suf-mobile"),
    document.querySelector(".texto-faca-parte"),
  ...
  elementsToAnimate.forEach((element) => {
    ...
    // Configurar opacidade inicial em 0 para transição mais suave
    element.style.opacity = "0";

    // Adicionar classe visible para iniciar a transição
    element.classList.add("visible");

    // Forçar um repaint para garantir que a transição seja aplicada
    element.offsetHeight;

    // Remover a configuração inline de opacidade para permitir que
    // a regra CSS .visible defina a opacidade para 1
    setTimeout(() => {
