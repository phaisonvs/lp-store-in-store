...
function triggerImmediateEntranceAnimation() {
  ...
  if (elements.length > 0) {
    ...
    setTimeout(() => {
      elements.forEach((el) => {
        // Garantir que começa sem a classe de visibilidade final
        el.classList.remove("is-visible");

        // Definir duração
        el.style.setProperty("--animate-duration", "2s");

        // Tornar visível ANTES de adicionar a classe de animação
        el.style.visibility = "visible";

        // Definir a classe de animação
        const animationClass = "animate__zoomIn"; // Mantenha a animação desejada
        ...
        // Adicionar listener para quando a animação terminar
        el.addEventListener(
          "animationend",
          () => {
            // Remover classes do Animate.css para limpar
            el.classList.remove("animate__animated", animationClass);
            // Adicionar classe para manter visível
            el.classList.add("is-visible");
            console.log(
              `Animação Animate.css finalizada e .is-visible adicionada para: ${el.className}`
            );
          },
          { once: true }
        ); // Garantir que o listener execute apenas uma vez
      });
    }, 50); // Manter delay?
  } else {
    console.warn(
