...
function triggerImmediateEntranceAnimation() {
  // Selecionando os elementos que queremos animar
  const deskSvg = document.querySelector(".svg-suf-desk");
  const mobileSvg = document.querySelector(".svg-suf-mobile");
  const textoFacaParte = document.querySelector(".texto-faca-parte");

  // Array com todos os elementos para processar de uma vez
  const elementsToAnimate = [deskSvg, mobileSvg, textoFacaParte].filter(
    (el) => el !== null
  );

  elementsToAnimate.forEach((element) => {
    if (element) {
      // Configurar o estado inicial: pouco visível mas não completamente invisível
      ...
      // Quando a animação terminar, garantir que fique 100% visível
