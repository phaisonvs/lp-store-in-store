...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicializar as animações dos SVGs e texto
  setupAnimations();
});

/**
 * @description Inicializa todas as animações da página
 */
function setupAnimations() {
  // Configurar animação dos SVGs
  setupSvgAnimations();
...
function setupSvgAnimations() {
  ...
  if (svgElements.length > 0) {
    // Adiciona a classe is-visible após um pequeno delay para garantir que os estilos CSS foram aplicados
    setTimeout(() => {
...
/**
 * @description Inicializa a animação de digitação
 */
function setupTypingAnimation() {
  const typingElement = document.querySelector(".typing-animation");

  if (typingElement) {
    // Conta o número de caracteres para ajustar a duração da animação
    const text = typingElement.textContent.trim();
    const charCount = text.length;

    // Calcula a duração total baseada na velocidade por caractere
    const typingSpeed =
      parseFloat(
    ...
    const totalDuration = charCount * typingSpeed;

    // Define atributos personalizados para a animação
    typingElement.style.setProperty("--character-count", charCount);
    typingElement.style.setProperty(
      "--total-typing-duration",
      `${totalDuration}s`
    );

    // Adiciona a classe is-typing após um pequeno delay
    setTimeout(() => {
