...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicializar o Slider de Prova Social (Splide.js)
  setupProvaSocialSlider();
});

/**
 * @description Inicializa o slider de prova social com Splide.js
 */
function setupProvaSocialSlider() {
  const sliderElement = document.querySelector(".prova-social-slider");
  if (sliderElement && typeof Splide !== "undefined") {
    try {
      new Splide(sliderElement, {
        type: "loop", // Loop infinito
        height: "180px", // Altura definida no CSS, pode ser redundante mas garante
        perPage: 2, // Quantos slides visíveis por vez (desktop)
        perMove: 1, // Mover 1 slide por vez
        gap: "1rem", // Espaço entre os slides
        pagination: true, // Mostrar paginação
        arrows: true, // Mostrar setas padrão
        autoplay: true, // Autoplay
        interval: 5000, // Intervalo do autoplay (5 segundos)
        pauseOnHover: true, // Pausar autoplay no hover
        breakpoints: {
          900: {
            // Abaixo de 900px
      ...
      console.log("Splide.js (Prova Social) inicializado.");
    } catch (error) {
      console.error("Erro ao inicializar Splide.js (Prova Social):", error);
    }
  } else if (!sliderElement) {
    console.error("Elemento .prova-social-slider não encontrado.");
  } else {
