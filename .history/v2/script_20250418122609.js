...
function triggerImmediateEntranceAnimation() {
  // Selecionar os elementos que serão animados
  const svgDeskElement = document.querySelector(".svg-suf-desk");
  const svgMobileElement = document.querySelector(".svg-suf-mobile");
  const textoElement = document.querySelector(".texto-faca-parte");

  // Criar array com os elementos (filtrando os que não existem)
  const elements = [svgDeskElement, svgMobileElement, textoElement].filter(
    Boolean
  );

  // Aplicar a animação em cada elemento
  elements.forEach((element) => {
    // Definir opacidade inicial de 20%
    element.style.opacity = "0.2";
    element.style.visibility = "visible";

    // Adicionar classe para controlar a transição
    element.classList.add("visible");

    // Remover classes Animate.css se estiverem sendo usadas anteriormente
    element.classList.remove("animate__animated", "animate__zoomIn");

    // Após a transição completa, garantir que a opacidade seja 100%
    setTimeout(() => {
      element.style.opacity = "1";
    }, 1500); // 1.5 segundos, igual à duração da transição
  });
}
