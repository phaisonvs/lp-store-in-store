  ...
  if (elements.length > 0) {
    setTimeout(() => {
      elements.forEach((el) => {
        ...
        // Adicionar classes Animate.css - MUDADO PARA fadeIn
        const animationClass = "animate__fadeIn";
        el.classList.add("animate__animated", animationClass);
        // Não definimos visibility aqui, pois opacity 0 no CSS já esconde

        console.log(
          `Animação Animate.css (${animationClass}) acionada para: ${el.className}`
        );

        // Remover classes após a animação e adicionar classe final para manter visível
        el.addEventListener(
          "animationend",
          () => {
            console.log(
              `Animação ${animationClass} finalizada para: ${el.className}`
            );
...
function setupEntranceAnimations() {
  ...
  const observerCallback = (entries, observer) => {
    entries.forEach((entry) => {
      ...
      const animationClassFadeInUp = "animate__fadeInUp"; // Classe para reutilizar

      // Lógica para .big-numbers (contador + animação CSS dos LIs)
      if (targetElement.classList.contains("big-numbers")) {
        ...
        if (isVisible) {
          ...
          listItems.forEach((item, index) => {
            item.classList.remove("animate__fadeOut"); // Limpa saída anterior
            item.style.visibility = "visible";
            item.classList.add("animate__animated", animationClassFadeInUp);
          });
        } else {
          listItems.forEach((item) => {
            item.classList.remove("animate__animated", animationClassFadeInUp);
            item.style.visibility = "hidden";
          });
        }
      }
      // Lógica para .container-prova-social-1 (Animate.css)
      else if (targetElement.classList.contains("container-prova-social-1")) {
        if (isVisible) {
          targetElement.style.visibility = "visible";
          targetElement.classList.add(
            "animate__animated",
            animationClassFadeInUp
          );
          console.log(
            `Animação ${animationClassFadeInUp} acionada para: Prova Social 1`
          );
        } else {
          ...
          console.log(
            `Animação ${animationClassFadeInUp} removida de: Prova Social 1`
          );
        }
        // Nota: Não adicionamos .animation-finished aqui,
        // pois queremos que ele desapareça se sair da tela.
      }
      // Lógica para outros elementos (classe .is-visible)
  ...
  // Adicionar .container-prova-social-1 aos elementos observados
  const elementsToObserve = document.querySelectorAll(
    ".big-numbers, .nossas-guides-container, .section-the-news, .carousel, .container-prova-social-1"
  );

  if (elementsToObserve.length > 0) {
    elementsToObserve.forEach((el) => {
      // Garantir estado inicial escondido para LIs e Prova Social
      ...
      } else if (el.classList.contains("container-prova-social-1")) {
        el.style.visibility = "hidden"; // Esconder prova social inicialmente
      }
      observer.observe(el);
    });
