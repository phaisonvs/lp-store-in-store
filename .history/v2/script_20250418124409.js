 ...
 *              Após a conclusão dessas animações, torna o carrossel de prova social visível.
 */
function triggerImmediateEntranceAnimation() {
  ...
  const provaSocialContainer = document.querySelector(
    ".container-prova-social-1"
  );

  if (elementsToAnimate.length > 0 && provaSocialContainer) {
    let animationsFinishedCount = 0;
    const totalAnimations = elementsToAnimate.length;

    const onAnimationEnd = () => {
      animationsFinishedCount++;
      if (animationsFinishedCount === totalAnimations) {
        console.log(
          "Todas as animações iniciais concluídas. Exibindo prova social."
        );
        provaSocialContainer.classList.add("prova-social-visivel");
        // Remover ouvintes para limpeza, já que 'once: true' foi usado
      }
    };

    // Pequeno delay opcional para garantir que os elementos estejam prontos
    setTimeout(() => {
      elementsToAnimate.forEach((el) => {
        ...
        // Adicionar ouvinte para o fim da animação (executa apenas uma vez)
        el.addEventListener("animationend", onAnimationEnd, { once: true });

        // Adicionar classes Animate.css para iniciar a animação
    ...
    console.warn(
      "Elementos para animação imediata ou container da prova social não encontrados."
    );
    // Se os elementos de animação não forem encontrados, mas o container de prova social sim,
    // talvez queiramos exibi-lo imediatamente ou após um delay fixo como fallback.
    if (provaSocialContainer && elementsToAnimate.length === 0) {
      console.log(
        "Elementos de animação inicial não encontrados, exibindo prova social diretamente após delay."
      );
      setTimeout(() => {
        provaSocialContainer.classList.add("prova-social-visivel");
      }, 500); // Exibe após 0.5s como fallback
    }
  }
}
