...
/**
 * @description Inicializa o parallax moderno com escala e translate no ::before
 */
function setupParallax() {
  ...
  const handleScroll = () => {
    ...
    // Posição do topo da viewport relativa ao topo da seção
    // Clamp garante que o valor não seja negativo antes da seção ou positivo demais depois
    const scrollPositionInSection = Math.max(
      0,
      Math.min(sectionHeight, scrollY - sectionTop + viewportHeight / 2)
    );

    // Progresso do scroll dentro da seção (0 a 1)
    const scrollProgress = scrollPositionInSection / sectionHeight;

    // Calcula Parallax Y (movimento mais lento)
    const parallaxSpeedY = 0.2; // Ajuste a velocidade do parallax vertical
    const yPos = -(scrollProgress * sectionHeight * parallaxSpeedY);

    // Calcula Parallax Scale (escala de 1.1 para 1)
    const startScale = 1.1;
    const endScale = 1.0;
    const scaleValue = startScale - (startScale - endScale) * scrollProgress;
