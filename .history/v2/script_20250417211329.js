...
/**
 * @description Inicializa o efeito de typing para o texto principal
 */
function setupTypingEffect() {
  const headingElement = document.querySelector(".maior-rede-de-acabamentos");

  if (headingElement) {
    // Definimos o texto original para garantir que ele seja exibido corretamente
    const textoOriginal = headingElement.textContent;

    // Aguardamos a animação dos SVGs terminar antes de iniciar o typing
    // A animação dos SVGs começa após 1s e dura aproximadamente 1.2s
    setTimeout(() => {
      // Configurar um background escuro adequado para o pseudoelemento
      const sectionBg = document.querySelector(".seja-um-franqueado");
      if (sectionBg) {
        // Capturar a cor de fundo atual para usar na animação
        const computedStyle = window.getComputedStyle(sectionBg);
        const backgroundColor = computedStyle.backgroundImage
          ? "rgba(51, 51, 51, 0.9)" // Fallback para seções com imagem de fundo
      ...
      // Adicionamos uma classe para ativar a animação
      headingElement.classList.add("typing-active");

      // Registramos o evento de fim da animação
      const handleAnimationEnd = (e) => {
        if (e.animationName === "typewriter") {
          console.log("Animação de typing concluída");
          // Removemos o listener para evitar chamadas repetidas
          headingElement.removeEventListener(
            "animationend",
            handleAnimationEnd
          );

          // Após o término da animação, podemos remover o pseudo-elemento
          // para que não atrapalhe a interação com o texto
          setTimeout(() => {
            headingElement.classList.add("typing-completed");
          }, 1000); // Deixamos o cursor piscando por mais 1 segundo
        }
      };

      headingElement.addEventListener("animationend", handleAnimationEnd);
    }, 2500); // 2.5 segundos após o carregamento da página
  }
}
