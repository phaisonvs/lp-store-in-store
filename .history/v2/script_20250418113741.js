...
document.addEventListener("DOMContentLoaded", function () {
  ...
  setupScrollFadeAnimation(".big-numbers ul li", {
    once: false,
    threshold: 0.1,
  }); // Anima sempre, threshold menor para listas
  setupScrollFadeAnimation(".section-the-news .card-main", {
    once: false,
    delay: 100,
  }); // Pequeno delay para o card principal
  setupScrollFadeAnimation(".section-the-news .card-secondary", {
    once: false,
    delay: 200,
  }); // Delay maior para cards secundários
  // Adicione aqui outros seletores se necessário, como o carrossel de guide shops:
  setupScrollFadeAnimation(".carousel .carousel__card", {
    once: false,
    threshold: 0.1,
  });
  // E o container de texto das guide shops
  setupScrollFadeAnimation(".nossas-guides-container h3", { once: false });
  setupScrollFadeAnimation(".nossas-guides-container .subtexto-guide", {
    once: false,
    delay: 150,
  });
...
function setupScrollFadeAnimation(selector, options = {}) {
  const elements = document.querySelectorAll(selector);
  if (!elements.length) {
    // console.warn(`Nenhum elemento encontrado para setupScrollFadeAnimation: ${selector}`); // Log opcional
    return;
  }

  // Opções padrão configuráveis
  const defaultOptions = {
    once: false, // Animar apenas na primeira vez que entra?
    delay: 0, // Atraso opcional para a animação de entrada
    enterTranslateY: [20, 0], // [De, Para] na entrada (efeito de subida)
    leaveTranslateY: [0, -20], // [De, Para] na saída (efeito de descida)
    enterDuration: 800,
    leaveDuration: 600,
    enterEasing: "easeOutQuad",
    leaveEasing: "easeInQuad",
    threshold: 0.15, // Porcentagem visível para acionar (0 a 1) - Ajustado para 15%
  };
  ...
  const observerCallback = (entries, observer) => {
    entries.forEach((entry) => {
      ...
      // Verifica o atributo data para controlar a lógica 'once'
      const hasAnimatedOnce = target.dataset.hasAnimated === "true";

      if (entry.isIntersecting) {
        // Animar entrada se 'once' for falso OU se for a primeira vez
        if (!config.once || !hasAnimatedOnce) {
          // Parar animações anteriores no mesmo elemento para evitar conflitos
          anime.remove(target);
          anime({
            targets: target,
            opacity: [0, 1], // Anima de 0 para 1
            translateY: config.enterTranslateY, // Anima a posição Y
            duration: config.enterDuration,
            easing: config.enterEasing,
            delay: anime.stagger(50, { start: config.delay }), // Stagger leve se houver múltiplos elementos
            begin: () => {
              // Garante que o estilo inicial é aplicado antes da animação,
              // especialmente se uma animação de saída foi interrompida.
              target.style.opacity = "0";
              target.style.transform = `translateY(${config.enterTranslateY[0]}px)`;
            },
            complete: () => {
              // Define o estado final explícito após a animação para garantir consistência
              target.style.opacity = "1";
              target.style.transform = "translateY(0px)"; // Ou translateY(config.enterTranslateY[1] + 'px')
            },
          });
          // Marca que a animação de entrada já ocorreu para a lógica 'once'
          target.dataset.hasAnimated = "true";

          // Se for 'once', desobserva o elemento após a primeira animação de entrada
          if (config.once) {
            observer.unobserve(target);
            // console.log('Unobserving (once: true):', target); // Log opcional
          }
        }
      } else {
        // Animar saída apenas se 'once' for falso E se a animação de entrada já ocorreu
        if (!config.once && hasAnimatedOnce) {
          // Parar animações anteriores no mesmo elemento
          anime.remove(target);
          anime({
            targets: target,
            opacity: [1, 0], // Anima de 1 para 0
            translateY: config.leaveTranslateY, // Anima a posição Y
            duration: config.leaveDuration,
            easing: config.leaveEasing,
            complete: () => {
              // Define o estado final explícito após a animação
              target.style.opacity = "0";
              target.style.transform = `translateY(${config.leaveTranslateY[1]}px)`;
              // Importante: Não resetar 'data-has-animated' aqui
              // senão a animação de entrada ocorreria novamente ao rolar de volta.
            },
          });
        }
      }
  ...
  // Cria uma única instância do observer para todos os elementos deste seletor
  const observer = new IntersectionObserver(observerCallback, {
    root: null, // Relativo à viewport
    rootMargin: "0px",
    threshold: config.threshold, // Usa o threshold configurado
  });

  // Define o estado inicial e começa a observar cada elemento
  elements.forEach((element) => {
    // Define estado inicial ANTES de observar para garantir que comecem escondidos
    element.style.opacity = "0";
    element.style.transform = `translateY(${config.enterTranslateY[0]}px)`;
    // Limpa o dataset caso a página seja recarregada (útil para testes)
    delete element.dataset.hasAnimated;
    observer.observe(element);
  });
  console.log(
    `ScrollFadeAnimation setup for ${elements.length} elements with selector: ${selector}`
  );
}
