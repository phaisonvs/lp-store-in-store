...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Configura a animação simultânea para os LIs de .big-numbers
  // (true = animar apenas uma vez)
  setupSimultaneousAnime(".big-numbers", "ul li", true);
});
 ...
 *              Para .big-numbers, anima os 'li' filhos simultaneamente com animejs.
...
function setupEntranceAnimations() {
  ...
  const observerCallback = (entries, observer) => {
    entries.forEach((entry) => {
      ...
      // Lógica específica para .big-numbers (agora usa animejs)
      if (targetElement.classList.contains("big-numbers")) {
        // A animação dos LIs é tratada por setupSimultaneousAnime
        // Mantém apenas a lógica de animação dos contadores aqui
        if (isVisible && !animatedCounters.has(targetElement)) {
          ...
          // Não desobservar, pois o observer ainda é útil se houver outras lógicas
        }
      }
      // Lógica para .nossas-guides-container (mantida)
      ...
      // Lógica para .section-the-news (mantida)
      ...
      // Lógica para .carousel (cards) (mantida)
  ...
  // Observa todos os elementos necessários (incluindo .big-numbers para os contadores)
...
/**
 * @description Configura animação simultânea para filhos de um container usando IntersectionObserver e animejs.
 * @param {string} containerSelector - Seletor CSS para o container pai.
 * @param {string} childSelector - Seletor CSS para os elementos filhos a serem animados.
 * @param {boolean} once - Se a animação deve ocorrer apenas uma vez.
 */
function setupSimultaneousAnime(containerSelector, childSelector, once = true) {
  ...
  const observerOptions = {
    root: null,
    rootMargin: "0px",
    threshold: 0.15, // Gatilho quando 15% do container está visível
  };

  let hasAnimated = false; // Controla se a animação 'once' já ocorreu
    ...
    entries.forEach((entry) => {
      if (entry.isIntersecting && (!once || !hasAnimated)) {
        ...
        if (children.length > 0) {
          // Garante que os elementos estejam no estado inicial (definido no CSS)
          // Poderia ser feito aqui também: children.forEach(el => { el.style.opacity = 0; el.style.transform = 'scale(0.95)'; });
          ...
          anime({
            ...
            duration: 700,
            easing: "easeOutQuad",
            // Adiciona um pequeno stagger para não ser *totalmente* simultâneo, dando um efeito mais orgânico.
            // Remova ou ajuste 'delay' se quiser 100% simultâneo.
            delay: anime.stagger(50), // Atraso de 50ms entre cada item
          });
          console.log(
            `Animação simultânea acionada para filhos de ${containerSelector}`
          );
          hasAnimated = true;
          if (once) {
            observer.unobserve(container);
            console.log(
              `Observer desativado para ${containerSelector} (once: true)`
            );
          }
        }
      } else if (!entry.isIntersecting && !once && hasAnimated) {
        // Opcional: reverter a animação ao sair da tela se 'once' for false
        const children = container.querySelectorAll(childSelector);
        if (children.length > 0) {
          anime({
            targets: children,
            opacity: 0,
            scale: 0.95,
            duration: 500, // Duração mais curta para sair
            easing: "easeInQuad",
            delay: 0, // Sem delay ao reverter
          });
          console.log(`Animação revertida para filhos de ${containerSelector}`);
          hasAnimated = false; // Permite animar novamente ao entrar
        }
      }
    });
  };
