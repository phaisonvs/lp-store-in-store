...
document.addEventListener("DOMContentLoaded", function () {
  // Inicialização do carrossel de Guide Shops (se ainda existir)
  const carousel = new Carousel(); // Supondo que este seja outro carrossel
  carousel.init();
  ...
  // Inicializar o Slider de Prova Social (Glide.js)
  setupProvaSocialGlide();
});

/**
 * @description Inicializa o slider de prova social com Glide.js
 */
function setupProvaSocialGlide() {
  const sliderElement = document.querySelector(".prova-social-glide");
  if (sliderElement && typeof Glide !== "undefined") {
    try {
      new Glide(sliderElement, {
        type: "carousel",
        startAt: 0,
        perView: 2.5, // Mostrar 2.5 slides em telas maiores
        focusAt: "center", // Centraliza o slide ativo, bom com perView fracionado
        gap: 20, // Aumentar um pouco o gap pode ficar melhor
        autoplay: 5000,
        hoverpause: true,
        breakpoints: {
          1024: {
            // Talvez ajustar breakpoint para 2 slides inteiros
            perView: 2,
          },
          768: {
            // Abaixo de 768px
            perView: 1.3, // Mostrar 1.3 em telas médias
            gap: 15,
            focusAt: 0, // Começar do início em telas menores
          },
          480: {
            // Abaixo de 480px
            perView: 1, // Mostrar 1 slide inteiro
            gap: 10,
            focusAt: 0,
          },
        },
      }).mount();
      console.log("Glide.js (Prova Social) inicializado com perView: 2.5");
    } catch (error) {
      console.error("Erro ao inicializar Glide.js (Prova Social):", error);
    }
  } else if (!sliderElement) {
    console.error("Elemento .prova-social-glide não encontrado.");
  } else {
    console.error("Biblioteca Glide não carregada.");
  }

  // Removemos a função antiga das setas manuais, se existir
  // removeArrowScrollSupport(); // Ou o nome da função antiga
}
