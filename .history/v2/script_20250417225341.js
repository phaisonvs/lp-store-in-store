...
function setupEntranceAnimations() {
  ...
  const observerCallback = (entries, observer) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        if (entry.target.classList.contains("big-numbers")) {
          const listItems = entry.target.querySelectorAll("ul li");
          listItems.forEach((item) => {
            item.classList.add("is-visible");
          });
          // Opcional: Desconectar o observer após a animação ocorrer uma vez
          // observer.unobserve(entry.target);
        }
        // Adicionar lógica para outros elementos observados aqui se necessário
      }
    });
  };
  ...
  // Seleciona os elementos que devem disparar animações
  const elementsToObserve = document.querySelectorAll(".big-numbers"); // Pode adicionar mais seletores aqui, ex: '.section-the-news'

  if (elementsToObserve.length > 0) {
    elementsToObserve.forEach((el) => observer.observe(el));
    console.log("IntersectionObserver configurado para animações de entrada.");
  } else {
    console.warn(
