...
/**
 * @description Configura IntersectionObserver para animações usando Animate.css
 */
function setupEntranceAnimations() {
  ...
  const observerCallback = (entries, observer) => {
    entries.forEach((entry) => {
      ...
      // Função auxiliar para aplicar animações Animate.css
      const applyAnimation = (element, animationIn, animationOut) => {
        // Limpar classes anteriores para evitar conflitos
        ...
        // Adicionar timeout para garantir que o navegador registre a remoção antes de adicionar
        setTimeout(() => {
          if (isVisible) {
            element.classList.add("animate__animated", animationIn);
          } else {
            // Só aplicar animação de saída se o elemento já esteve visível
            if (element.dataset.hasBeenVisible) {
              element.classList.add("animate__animated", animationOut);
            }
          }
          // Marcar que o elemento esteve visível (para animação de saída)
          if (isVisible) {
            element.dataset.hasBeenVisible = true;
          } else {
            // Opcional: remover a marcação se sair, para reanimar sempre
            // delete element.dataset.hasBeenVisible;
          }
        }, 10); // Pequeno delay
      };
      ...
      if (targetElement.classList.contains("big-numbers")) {
        ...
        listItems.forEach((item) =>
          applyAnimation(item, "animate__fadeInUp", "animate__fadeOut")
        );
        ...
        if (title)
          applyAnimation(title, "animate__fadeInUp", "animate__fadeOut");
        if (subtitle)
          applyAnimation(subtitle, "animate__fadeInUp", "animate__fadeOut");
        ...
        cards.forEach((card) =>
          applyAnimation(card, "animate__fadeInUp", "animate__fadeOut")
        );
      } else if (targetElement.classList.contains("carousel")) {
        const cards = targetElement.querySelectorAll(
          ".carousel__card:not(.clone)"
        ); // Ignorar clones
        // Aplicar animação aos cards originais. O carrossel cuida dos clones.
        cards.forEach((card) =>
          applyAnimation(card, "animate__fadeIn", "animate__fadeOut")
        ); // Usar fadeIn simples para cards
      }
    });
  };
  ...
  if (elementsToObserve.length > 0) {
    ...
    console.log(
      "IntersectionObserver configurado para Animate.css:",
      elementsToObserve
    );
  } else {
...
function animateCounters(elements) {
  ...
  elements.forEach((counterElement) => {
    ...
    // Obter o valor final do contador (limpar caracteres não numéricos para o cálculo)
    let finalValue = originalText;
    let prefix = "";
    let suffix = "";

    // Detectar prefixos como +, -
    if (finalValue.startsWith("+")) {
      prefix = "+";
      finalValue = finalValue.substring(1);
    }

    // Detectar sufixos como x, %
    if (finalValue.endsWith("x")) {
      suffix = "x";
    ...
    // Se o valor for muito grande, aumentar o incremento para evitar atrasos
    ...
    const animate = () => {
      ...
      // Aplicar easing para desacelerar no final
      currentValue = Math.min(numericValue * easedProgress, numericValue);

      // Arredondar para inteiro se o valor final for inteiro
      const displayValue = Number.isInteger(numericValue)
        ? Math.floor(currentValue)
        : currentValue.toFixed(1);

      // Atualizar o texto com prefixo/sufixo se existirem
      counterElement.textContent = `${prefix}${displayValue}${suffix}`;
