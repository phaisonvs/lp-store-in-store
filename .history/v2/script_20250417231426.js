...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Configura o IntersectionObserver para animar elementos ao entrar/sair da viewport.
...
function setupScrollFadeAnimations() {
  const elementsToAnimate = document.querySelectorAll(
    // Seleciona filhos diretos específicos do container principal
    ".conteudo-seja-um-franqueado-abc > .svg-suf-desk, " +
      ".conteudo-seja-um-franqueado-abc > .svg-suf-mobile, " +
      ".conteudo-seja-um-franqueado-abc > .faca-parte, " +
      ".conteudo-seja-um-franqueado-abc > .prova-social-slider, " +
      ".conteudo-seja-um-franqueado-abc > .big-numbers, " +
      ".conteudo-seja-um-franqueado-abc > .section-the-news, " +
      ".conteudo-seja-um-franqueado-abc > .nossas-guides-container, " +
      ".conteudo-seja-um-franqueado-abc > .carousel"
    // Adicione outras classes de filhos diretos se necessário
  );

  if (!elementsToAnimate.length) {
    console.warn("Nenhum elemento encontrado para animação de scroll fade.");
    return;
  }

  const observerOptions = {
    root: null, // Observa em relação à viewport
    rootMargin: "0px 0px -10% 0px", // Trigger um pouco antes de estar 100% visível
  ...
  const observerCallback = (entries, observer) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        // Remove a classe para permitir animação de saída
  ...
  elementsToAnimate.forEach((el) => {
    el.classList.add("animate-on-scroll"); // Adiciona a classe base de animação
