...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicializar o efeito de typing
  setupTypingEffect();
});

/**
 * @description Inicializa o efeito de typing para o texto principal usando ::before e clip-path
 */
function setupTypingEffect() {
  const headingElement = document.querySelector(".maior-rede-de-acabamentos");

  if (headingElement) {
    // Evita reinicializar
    if (
      headingElement.classList.contains("typing-active") ||
      headingElement.classList.contains("typing-completed")
    ) {
      return;
    }

    const textoOriginal = headingElement.textContent
      .trim()
      .replace(/\s+/g, " ");
    const numeroCaracteres = textoOriginal.length;

    // Configura dados para o CSS
    headingElement.setAttribute("data-text", textoOriginal);
    headingElement.style.setProperty("--char-count", numeroCaracteres);

    // Limpa o texto visível original (opcional, pois a cor é transparent)
    ...
    setTimeout(() => {
      ...
      const handleAnimationEnd = (e) => {
        // Verifica a animação no elemento PAI, pois ::before não dispara eventos
        // A animação que queremos monitorar é 'typing-reveal' (aplicada implicitamente via classe)
        // No entanto, 'animationend' no pai pode ser complicado de sincronizar com o ::before.
        // Uma abordagem mais robusta é usar um segundo setTimeout baseado na duração da animação CSS.

        // Removemos o listener para não ser chamado por outras animações
        headingElement.removeEventListener("animationend", handleAnimationEnd);

        // Usamos setTimeout para marcar como completo após a duração da animação
        // Isso é mais confiável do que depender do evento animationend do pai para o pseudo-elemento
        const typingDuration =
          parseFloat(
            getComputedStyle(headingElement).getPropertyValue("--typing-speed")
          ) * 1000;
        const mobileMultiplier = window.innerWidth <= 800 ? 0.7 : 1;
        const effectiveDuration = typingDuration * mobileMultiplier;

        console.log(
          `Aguardando ${effectiveDuration}ms para completar o typing.`
        );

        setTimeout(() => {
          console.log("Marcando typing como completo via setTimeout.");
          headingElement.classList.remove("typing-active");
          headingElement.classList.add("typing-completed");
        }, effectiveDuration);
      };

      // Adiciona o listener, mas a lógica principal agora usa setTimeout
      headingElement.addEventListener("animationend", handleAnimationEnd);

      // Gatilho inicial para o caso de não haver outras animações no elemento
      // (se houver, o animationend será capturado acima)
      // Contudo, a abordagem com setTimeout abaixo é preferível.
      // handleAnimationEnd({ animationName: 'typing-reveal' }); // Simula o fim para teste
    }, 2500); // Delay inicial
  }
}
