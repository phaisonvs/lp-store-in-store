...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicializar o Simple Parallax JS
  setupSimpleParallax();
});

/**
 * @description Inicializa a biblioteca simple-parallax-js na seção principal
 */
function setupSimpleParallax() {
  const element = document.querySelector(".seja-um-franqueado");
  if (element && typeof simpleParallax !== "undefined") {
    // Verifica se a lib carregou
    try {
      new simpleParallax(element, {
        overflow: true, // Geralmente necessário para background images
        scale: 1.2, // Um leve scale pode modernizar, ajuste conforme necessário
        delay: 0.4, // Um pequeno delay para suavizar
      });
      console.log("Simple Parallax inicializado em", element);
    } catch (error) {
      console.error("Erro ao inicializar Simple Parallax:", error);
    }
  } else if (!element) {
    console.error(
      "Elemento .seja-um-franqueado não encontrado para Simple Parallax."
    );
  } else {
    console.error("Biblioteca simpleParallax não carregada.");
  }
}
