...
function triggerImmediateEntranceAnimation() {
  ...
  ].filter((el) => el !== null); // Filtra elementos que não existem

  // Remove qualquer classe de animação anterior
  elements.forEach((el) => {
    el.classList.remove("animate__animated", "animate__zoomIn");

    // Define a opacidade inicial como 20% e torna o elemento visível
    el.style.opacity = "0.2";
    el.classList.add("visible");

    // Após um pequeno delay, inicia a transição para opacidade 100%
    setTimeout(() => {
      el.style.opacity = "1";
    }, 50);
  });
