...
/**
 * @description Inicializa o efeito de typing para o texto principal usando pseudo-elemento
 */
function setupTypingEffect() {
  const headingElement = document.querySelector(".maior-rede-de-acabamentos");

  if (headingElement) {
    // Evita reinicializar a animação
    if (
    ...
    // Pega o texto, remove espaços extras e calcula caracteres
    const textoOriginal = headingElement.textContent
      .trim()
      .replace(/\s+/g, " ");
    const numeroCaracteres = textoOriginal.length;

    // Define o texto no atributo data-text para o ::before usar
    headingElement.setAttribute("data-text", textoOriginal);
    // Atualiza a variável CSS com o número de caracteres
    headingElement.style.setProperty("--char-count", numeroCaracteres);

    // Limpa o conteúdo visível do elemento principal
    // headingElement.textContent = ''; // Opcional, já que a cor é transparente

    // Aguarda a animação dos SVGs terminar
    setTimeout(() => {
      // Adiciona a classe para iniciar a animação no ::before
      headingElement.classList.add("typing-active");

      // Monitora o fim da animação no pseudo-elemento
      const pseudoElement = window.getComputedStyle(headingElement, "::before");

      const handleAnimationEnd = (e) => {
        // Verifica se a animação de typing terminou no pseudo-elemento
        if (e.animationName === "typing") {
          console.log("Animação de typing (::before) concluída");
          headingElement.removeEventListener(
            "animationend",
            handleAnimationEnd
          );

          // Transfere as classes de estado
          headingElement.classList.remove("typing-active");
          headingElement.classList.add("typing-completed");
        }
      };

      // Adiciona o listener ao elemento pai, pois pseudo-elementos não disparam eventos
