  ...
  const observerCallback = (entries, observer) => {
    entries.forEach((entry) => {
      ...
      if (targetElement.classList.contains("big-numbers")) {
        ...
        if (isVisible) {
          // Animar contadores apenas na primeira vez que o container fica visível
          if (!animatedCounters.has(targetElement)) {
            console.log(
              "Iniciando animação para contadores em:",
              targetElement
            );
          ...
          // Esconder LIs quando saem da view
          listItems.forEach((item) => {
            item.classList.remove("animate__animated", "animate__fadeInUp");
            item.style.visibility = "hidden";
          });
          // Opcional: remover o container do Set para reanimar se voltar à view?
          // animatedCounters.delete(targetElement);
        }
      }
