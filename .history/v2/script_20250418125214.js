...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicializar a funcionalidade de placeholder dos iframes de prova social
  setupIframePlaceholders();
});
...
/**
 * @description Adiciona listeners aos placeholders para carregar iframes ao clicar.
 */
function setupIframePlaceholders() {
  const carousel = document.querySelector(".carrossel-prova-social-1");

  if (!carousel) {
    console.warn(
      "DEBUG: Carrossel de prova social não encontrado para placeholders."
    );
    return;
  }

  console.log("DEBUG: Adicionando listener de clique ao carrossel:", carousel);

  carousel.addEventListener("click", function (event) {
    console.log("DEBUG: Clique detectado no carrossel. Target:", event.target);
    // Encontra o placeholder clicado (ou um de seus filhos)
    const placeholder = event.target.closest(".iframe-placeholder");
    console.log("DEBUG: Placeholder encontrado:", placeholder);

    if (placeholder && placeholder.dataset.src) {
      const iframeSrc = placeholder.dataset.src;
      console.log("DEBUG: iframeSrc encontrado:", iframeSrc);
      const card = placeholder.closest(".card-prova-social");
      console.log("DEBUG: Card pai encontrado:", card);

      if (card) {
        // Criar o iframe
        const iframe = document.createElement("iframe");
        iframe.setAttribute("src", iframeSrc);
        iframe.setAttribute("title", "YouTube video player"); // Título genérico, pode ser melhorado
        ...
        iframe.setAttribute("loading", "lazy"); // Manter o lazy loading

        console.log("DEBUG: Iframe criado:", iframe);

        // Limpar o conteúdo do card e adicionar o iframe
        card.innerHTML = ""; // Remove o placeholder
        card.appendChild(iframe);
        console.log("DEBUG: Iframe adicionado ao card.");
      } else {
        console.warn(
          "DEBUG: Card pai não encontrado para o placeholder:",
          placeholder
        );
      }
    } else {
      console.log("DEBUG: Placeholder não encontrado ou sem data-src.");
    }
  });

  // Adicionar listener para tecla Enter/Space para acessibilidade
  carousel.addEventListener("keydown", function (event) {
    const placeholder = event.target.closest(
      ".iframe-placeholder[tabindex='0']"
    );
    console.log(
      "DEBUG: Keydown detectado. Target:",
      event.target,
      "Key:",
      event.key
    );
    if (placeholder && (event.key === "Enter" || event.key === " ")) {
      console.log(
        "DEBUG: Tecla Enter/Space pressionada no placeholder:",
        placeholder
      );
      event.preventDefault(); // Previne scroll da página com Space
      placeholder.click(); // Simula o clique
    }
  });

  console.log("DEBUG: Placeholders de iframe configurados.");
}
