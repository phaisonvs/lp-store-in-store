...
function animateCounters(elements) {
  ...
  elements.forEach((counterElement) => {
    ...
    const duration = 4500; // duração em ms (Aumentado de 1500 para 2500)
    ...
    const animate = () => {
      ...
      // Formatar o número para exibição com separador de milhares (ponto no pt-BR)
      let formattedDisplayValue;
      if (Number.isInteger(numericValue)) {
        formattedDisplayValue =
          Math.floor(currentValue).toLocaleString("pt-BR");
      } else {
        // Para números não inteiros, formatamos após fixar a casa decimal
        // A formatação padrão pode não ser ideal aqui, dependendo do número
        // Vamos formatar a parte inteira e manter a decimal separada por vírgula
        const parts = currentValue.toFixed(1).split(".");
        parts[0] = parseInt(parts[0]).toLocaleString("pt-BR");
        formattedDisplayValue = parts.join(",");
      }

      // Atualizar o texto com prefixo/sufixo e valor formatado
