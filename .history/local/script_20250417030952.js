...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupGuideShopCarouselEventListeners = (carousel, track) => {
    ...
    const pointerStart = (e) => {
      ...
      state.guideShopCarousel.startTime = Date.now();
      state.guideShopCarousel.lastPosition =
        state.guideShopCarousel.startPosition;
    ...
    const pointerMove = (e) => {
      ...
      const timeDiff = currentTime - state.guideShopCarousel.startTime;
      const distance = currentPosition - state.guideShopCarousel.lastPosition;
      state.guideShopCarousel.velocity = distance / timeDiff;

      // Atualizar posição e tempo para próximo cálculo
      state.guideShopCarousel.lastPosition = currentPosition;
      state.guideShopCarousel.startTime = currentTime;
    ...
    const applyMomentum = () => {
      const velocity = state.guideShopCarousel.velocity;
      const deceleration = 0.95; // Fator de desaceleração
      ...
      const animate = () => {
        if (
          Math.abs(currentVelocity) < minVelocity ||
          !state.guideShopCarousel.isDragging
        ) {
        ...
        state.guideShopCarousel.currentTranslate += currentVelocity * 16; // 16ms é aproximadamente um frame
    ...
    const endDrag = () => {
      ...
      if (Math.abs(state.guideShopCarousel.velocity) > 0.5) {
        applyMomentum();
      } else {
        handleDragEnd(carousel, track);
      }
  ...
  /**
   * @description Inicializa o slider de franquias
   */
  const initFranquiasSlider = () => {
    const slider = document.querySelector(".slider-franquias");
    const track = document.querySelector(".slider-franquias__track");
    const prevButton = document.querySelector(".slider-franquias__nav--prev");
    const nextButton = document.querySelector(".slider-franquias__nav--next");

    if (!slider || !track || !prevButton || !nextButton) return;

    // Estado local do slider
    const state = {
      isDragging: false,
      startPosition: 0,
      currentTranslate: 0,
    ...
    // Configurar lazy loading dos vídeos
    ...
    const getSlideWidth = () => {
      const card = track.querySelector(".slider-franquias__card");
      return card
        ? card.offsetWidth + parseInt(getComputedStyle(track).gap)
        : 0;
    };

    // Funções de navegação
    const updateSlidePosition = (position) => {
      state.currentTranslate = position;
      requestAnimationFrame(() => {
    ...
    const moveToSlide = (index) => {
      ...
      const position = -state.currentIndex * slideWidth;
    ...
    const handleDragStart = (e) => {
      ...
      state.startPosition = e.type.includes("mouse")
        ? e.pageX
        : e.touches[0].pageX;
    ...
    const handleDragMove = (e) => {
      ...
      const distance = currentPosition - state.lastPosition;
      state.velocity = distance / timeDiff;

      state.lastPosition = currentPosition;
      state.startTime = currentTime;

      const diff = currentPosition - state.startPosition;
      const newTranslate = state.prevTranslate + diff;

      updateSlidePosition(newTranslate);
    };
    ...
    const handleDragEnd = () => {
      ...
      const slideWidth = getSlideWidth();
      const moveBy = state.velocity * 200; // Multiplicador de momentum
      const targetTranslate = state.currentTranslate + moveBy;
    ...
    prevButton.addEventListener("click", () => {
      prevButton.classList.add("active");
      moveToSlide(state.currentIndex - 1);
    ...
    const updateButtonsVisibility = () => {
      ...
      nextButton.style.pointerEvents =
        state.currentIndex === maxIndex ? "none" : "auto";
    };

    // Observer para atualizar em mudanças de layout
    const resizeObserver = new ResizeObserver(() => {
      moveToSlide(state.currentIndex);
