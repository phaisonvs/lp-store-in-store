...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupLazyVideos = (container) => {
    ...
    const videoObserver = new IntersectionObserver(
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (iframe.dataset.src) {
              ...
              // Remover data-src para evitar recarregamento
      ...
      {
        rootMargin: "50px", // Carregar um pouco antes de entrar no viewport
        threshold: 0.1,
      }
    );
    ...
    videoFrames.forEach((iframe) => {
      // Garantir que o iframe tenha a classe lazy-video e data-src
      if (!iframe.dataset.src) {
  ...
  const init = () => {
    ...
    setupVideoCarouselControls(); // Mover para componentes críticos
