...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const otherElements = document.querySelectorAll(
    ".section-the-news, .nossas-guides-container, .carousel, .svg-suf-desk, .svg-suf-mobile, .faca-parte, .big-numbers ul li"
  );
  ...
  function initVideoSlider() {
    ...
    function goToSlide(index) {
      ...
      // Não mudar de slide se estiver assistindo um vídeo (prevenção de mudança acidental)
      if (isVideoPlaying && index !== currentIndex) {
        const userConfirmed = window.confirm(
          "Deseja realmente mudar de vídeo?"
        );
        if (!userConfirmed) {
          return;
        }
      ...
      slides.forEach((slide) => {
        ...
        if (iframe && iframe.src.includes("youtube.com")) {
          // Salvamos e restauramos o src para pausar o vídeo
    ...
    document.addEventListener("keydown", (e) => {
      // Verificar se o foco está no slider
      if (
    ...
    // Detectar quando um vídeo está sendo reproduzido
    slides.forEach((slide) => {
      slide.addEventListener("click", () => {
        // Marcar quando o usuário interage com o vídeo
        setTimeout(() => {
          isVideoPlaying = true;
        }, 200);
      });
    ...
    // Acessibilidade: garantir que os botões de navegação sejam tabuláveis
