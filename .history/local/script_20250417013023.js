...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupVideoCarouselControls = () => {
    ...
    console.log("Carrossel de vídeos inicializado");

    // Garantir que os iframes estejam disponíveis
    const videoCards = carousel.querySelectorAll(".card-prova-social");

    if (videoCards.length === 0) {
      console.log("Nenhum card de vídeo encontrado");
      return;
    }

    // Calcular scroll amount uma única vez
    const cardWidth = videoCards[0].offsetWidth || 320;
    ...
    // Remover função de lazy loading para garantir que os vídeos apareçam
    videoCards.forEach((card) => {
      const iframe = card.querySelector("iframe");
      if (iframe) {
        // Garantir que o iframe tenha src definido
  ...
  /**
   * @description Configura lazy loading para vídeos - função simplificada
   * @param {HTMLElement} container - Contêiner dos vídeos
   */
  const setupLazyVideos = (container) => {
    // Função simplificada para não interferir com os vídeos
    console.log("Lazy loading de vídeos está desativado para evitar problemas");
  };
