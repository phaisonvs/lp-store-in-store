...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupVideoCarouselControls = () => {
    ...
    const cardWidth = videos[0]?.offsetWidth || 0;
    const scrollAmount = cardWidth + 20; // largura + margem
    console.log("Largura dos cards:", cardWidth);

    // Adicionar atributos de acessibilidade
    prevButton.setAttribute("role", "button");
    prevButton.setAttribute("aria-label", "Vídeo anterior");
    prevButton.setAttribute("tabindex", "0");

    nextButton.setAttribute("role", "button");
    nextButton.setAttribute("aria-label", "Próximo vídeo");
    nextButton.setAttribute("tabindex", "0");

    // Eventos de navegação otimizados para clique
    prevButton.addEventListener("click", () => {
    ...
    // Otimizar para touch devices - detectar swipes
    let touchStartX = 0;
    let touchEndX = 0;
    ...
    const handleSwipe = () => {
      ...
      // Definir um threshold mínimo para considerar como swipe intencional
      const minSwipeDistance = 50;

      if (swipeDistance > minSwipeDistance) {
        // Swipe para direita - voltar para o vídeo anterior
        carousel.scrollBy({
          left: -scrollAmount,
          behavior: "smooth",
        });
      } else if (swipeDistance < -minSwipeDistance) {
        // Swipe para esquerda - ir para o próximo vídeo
        carousel.scrollBy({
          left: scrollAmount,
          behavior: "smooth",
        });
