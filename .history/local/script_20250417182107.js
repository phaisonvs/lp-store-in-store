...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /**
   * @description Mostra todos os elementos sem animação - substituindo a lógica de animação anterior
   */
  const showAllElements = () => {
    // Tornar todos os elementos com animação imediatamente visíveis
    ...
    // Garantir que seções específicas estejam visíveis
    const sections = [
  ...
  /**
   * @description Função principal - inicializa todos os componentes
   */
  const init = () => {
    showAllElements();
    setupParallax();
  ...
  // Manter visível ao alternar abas ou voltar à aplicação
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) {
      // Garantir que todo o conteúdo permaneça visível
      showAllElements();
    }
  });
});
