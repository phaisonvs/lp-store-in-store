...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupScrollAnimations = () => {
    ...
    // Remove container-prova-social-1 das animações
    state.animations.sections = state.animations.sections.filter(
      selector => selector !== '.container-prova-social-1'
    );

    // Remover classes de animação do container-prova-social-1 se existirem
    const container = document.querySelector('.container-prova-social-1');
    if (container) {
      container.classList.remove('animate-on-scroll');
    ...
    state.animations.observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        // Não animar se for o container-prova-social-1
        if (entry.target.classList.contains('container-prova-social-1')) {
          return;
        }
    ...
    requestAnimationFrame(() => {
      document
        ...
        .forEach((element) => {
          if (!element.classList.contains('container-prova-social-1')) {
...
// Carrossel de Franquias
function initFranquiasCarousel() {
  const track = document.querySelector(".section-franquias__track");
  const prevButton = document.querySelector(".section-franquias__nav--prev");
  const nextButton = document.querySelector(".section-franquias__nav--next");
/**
 * @description Script principal otimizado para performance da página "Seja um Franqueado"
 * @author Dev ABC da Construção
 */
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupScrollAnimations = () => {
    ...
    // Criar um IntersectionObserver eficiente com opções otimizadas
    const observerOptions = {
      root: null,
      rootMargin: "50px 0px", // Pre-carrega elementos um pouco antes
      threshold: [0, 0.1, 0.2], // Pontos de trigger mais suaves
    };

    // Performance optimization: Use um Map para rastrear elementos já animados
    ...
    state.animations.observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        ...
        const wasAnimated = animatedElements.get(target);
        ...
        if (entry.isIntersecting) {
          ...
          if (!wasAnimated) {
            ...
            // Otimização: Para elementos que só precisam animar uma vez
            if (target.dataset.animateOnce === "true") {
              state.animations.observer.unobserve(target);
            }
          }
        } else if (!target.dataset.animateOnce) {
          // Remove a classe apenas se o elemento não for "animate-once"
          requestAnimationFrame(() => {
            target.classList.remove("is-visible");
            animatedElements.set(target, false);
          });
        }
      });
    }, observerOptions);

    // Otimização: Usa fragment para batch DOM updates
    const fragment = document.createDocumentFragment();
    ...
    elementsToAnimate.forEach((element) => {
      // Adiciona classe base e configurações
      element.classList.add("animate-on-scroll");

      // Configura elementos que só devem animar uma vez
      if (element.matches(".svg-suf-desk, .svg-suf-mobile, .faca-parte")) {
        element.dataset.animateOnce = "true";
      }

      // Adiciona ao fragment para batch update
    ...
    requestAnimationFrame(() => {
      elementsToAnimate.forEach((element, index) => {
        const clone = fragment.children[index];
        element.className = clone.className;
        element.dataset.animateOnce = clone.dataset.animateOnce;
...
function initFranquiasCarousel() {
  ...
  const prevButton = document.querySelector(".section-franquias__nav--prev");
  const nextButton = document.querySelector(".section-franquias__nav--next");
  const cards = track.querySelectorAll(".section-franquias__card");
  ...
  // Previne comportamento padrão de drag de imagens
  cards.forEach((card) => {
  ...
  function stopDragging() {
    ...
    // Se moveu significativamente, mova para o próximo/anterior
  ...
  function moveCarousel(direction) {
    const cardWidth = cards[0].offsetWidth + 24; // Inclui o gap
  ...
  // Atualiza botões de navegação quando a janela é redimensionada
