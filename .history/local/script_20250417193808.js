...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initVideoSlider() {
    ...
    const header =
      document.querySelector(".video-section-header") ||
    ...
    const arrowsContainer =
      document.querySelector(".custom-arrows-container") ||
      document.createElement("div");
    ...
    if (originalPrev && originalNext) {
      ...
      // Adicionar o container ao cabeçalho se ainda não estiver lá
      if (!arrowsContainer.parentNode) {
        header.appendChild(arrowsContainer);
      }
    }

    // Lazy loading para vídeos
    const videos = videoSlider.querySelectorAll("video[data-src]");
  ...
  // Inicializar o slider de vídeos diretamente
  initVideoSlider();
  ...
  // Deixar outros elementos importantes visíveis
  const otherElementsToShow = [
  ...
  otherElementsToShow.forEach((selector) => {
