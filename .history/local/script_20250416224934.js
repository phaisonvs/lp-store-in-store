      ...
      styles.forEach((style) => {
        ...
        button.addEventListener("click", () => {
          ...
          if (style.isZoom) {
            ...
            if (zoomActive) {
              // Garantir que a transição esteja ativa antes de reativar o zoom
              section.style.transition =
                "background-size 0.5s cubic-bezier(0.25, 0.1, 0.25, 1)";
              setupParallaxZoomEffect();
            } else {
              // Remover evento de scroll e redefinir tamanho com transição suave
              window.removeEventListener("scroll", updateZoom);
              // Manter a transição ao desativar para ter um efeito suave
              section.style.backgroundSize = "105%";
            }
            return;
          }
          ...
          if (zoomActive) {
            ...
            window.removeEventListener("scroll", updateZoom);
            // Garantir tamanho padrão ao mudar para outro estilo
            section.style.backgroundSize = ""; // Remover inline style para usar o do CSS
          }
        });
