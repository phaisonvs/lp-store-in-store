...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const state = {
    ...
    specialFadeIn: {
      elements: [".svg-suf-desk", ".svg-suf-mobile", ".faca-parte"],
      initiated: false,
    },
    isInitialized: false,
  };
  ...
  /**
   * @description Inicializa as animações especiais de fade-in
   * @returns {void}
   */
  const setupSpecialFadeIn = () => {
    if (state.specialFadeIn.initiated) return;

    // Encontrar o elemento visível entre SVG desk e mobile
    const svgDesk = document.querySelector(".svg-suf-desk");
    const svgMobile = document.querySelector(".svg-suf-mobile");
    const facaParte = document.querySelector(".faca-parte");

    const elements = [
      window.innerWidth <= 800 ? svgMobile : svgDesk,
      facaParte,
    ].filter((el) => el !== null);

    // Iniciar as animações com um pequeno atraso para garantir suavidade
    setTimeout(() => {
    ...
    state.specialFadeIn.initiated = true;
  };
  ...
  const init = () => {
    ...
    // Configurar visibilidade inicial para os elementos com fade-in especial
    const specialElements = document.querySelectorAll(
      ".svg-suf-desk, .svg-suf-mobile, .faca-parte"
    );
    ...
    // Verificar elementos no viewport inicial
    setTimeout(() => {
      checkElementsInViewport();
    }, 300);
