...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupLazyVideos = (container) => {
    ...
    videoFrames.forEach((iframe, index) => {
      // Para os primeiros 2 vídeos visíveis, carregue imediatamente
      if (index < 2) {
        // Se o iframe já tem o data-src, use-o, senão use o src atual
        if (iframe.hasAttribute("data-src")) {
          iframe.src = iframe.getAttribute("data-src");
        } else if (iframe.src === "about:blank" || iframe.src === "") {
          // Recuperar a URL do atributo data-src se o src estiver vazio ou for about:blank
        ...
        // Para os outros vídeos, implementar lazy loading
        // Se o iframe já tem um src válido e não é about:blank, preserve-o em data-src
        const currentSrc = iframe.src;
        if (currentSrc && currentSrc !== "about:blank" && currentSrc !== "") {
          iframe.dataset.src = currentSrc;
          iframe.src = "about:blank"; // Substituir temporariamente por página vazia
        }
    ...
    // Observar apenas os vídeos marcados para lazy loading
    videoFrames.forEach((iframe, index) => {
      if (index >= 2) {
        // Não observe os primeiros dois vídeos já carregados
  ...
  const init = () => {
    ...
    // Inicializar carrossel de vídeos imediatamente para melhorar experiência
