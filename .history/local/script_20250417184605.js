...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initVideoSlider() {
    ...
    function goToSlide(index) {
      ...
      // Calcula posição do slide com o gap
      const offset = -(currentIndex * (slideWidth + slideGap));
      slider.style.transform = `translateX(${offset}px)`;

      // Atualizar a paginação
      paginationDots.forEach((dot, i) => {
        // Ajusta dots para representar "páginas" em vez de slides individuais
        const dotPage = Math.floor(i / slidesToShow);
    ...
    if (paginationDots.length > 0) {
      // Ajusta a exibição de dots baseada no número de "páginas"
      const totalPages = Math.ceil(slides.length / slidesToShow);

      // Mostra apenas dots necessários
