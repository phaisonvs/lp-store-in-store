...
const animate = (track) => {
  ...
  const { currentTranslate, scrollSpeed, cardWidth } = state.guideShopCarousel;
  const cards = track.querySelectorAll(".carousel__card");
...
// Garantir que todos os elementos sejam visíveis imediatamente
const allElementsToShow = document.querySelectorAll(
  ".animate-on-scroll, .container-prova-social-1, .section-the-news, .nossas-guides-container, .carousel, .card-prova-social, .section-franquias, .section-content, .section-perguntas-frequentes, .section-blog-posts, .clientes-dizem-section, .section-banner-form, .svg-suf-desk, .svg-suf-mobile, .faca-parte, .big-numbers ul li, .video-slider-container"
);

allElementsToShow.forEach((el) => {
  if (el) {
    el.style.opacity = "1";
