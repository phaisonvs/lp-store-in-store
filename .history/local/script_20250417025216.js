...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const state = {
    guideShopCarousel: {
      ...
      autoplayEnabled: true, // Novo estado para controlar autoplay
      autoplayPaused: false, // Novo estado para pausar temporariamente
    },
    animations: {
      observer: null,
      initialized: false,
   ...
   */
  const animate = (track) => {
    // Se estiver em transição, dragging ou autoplay pausado, não continuar a animação
    if (
      state.guideShopCarousel.isTransitioning ||
      state.guideShopCarousel.isDragging ||
      state.guideShopCarousel.autoplayPaused ||
      !state.guideShopCarousel.autoplayEnabled
    ) {
      return;
    }
  ...
  const setupGuideShopCarouselEventListeners = (carousel, track) => {
    ...
    const pointerStart = (e) => {
      ...
      state.guideShopCarousel.autoplayPaused = true; // Pausa o autoplay durante o drag
    ...
    const endDrag = () => {
      ...
      state.guideShopCarousel.autoplayPaused = false; // Retoma o autoplay após o drag
