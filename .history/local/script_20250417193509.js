...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initVideoSlider() {
    const videoSplide = document.getElementById("video-splide");

    if (!videoSplide) return;
    ...
    const splide = new Splide("#video-splide", {
      ...
      arrowPath: "m13.5 26.5-10-10 10-10M14.5 6.5l10 10-10 10", // Caminho SVG personalizado para as setas
    }).mount();

    // Mover os botões de navegação para o container personalizado
    const arrowsContainer = document.querySelector(".custom-arrows-container");
    if (arrowsContainer) {
      const arrows = videoSplide.querySelectorAll(".splide__arrow");
      if (arrows.length) {
        Array.from(arrows).forEach((arrow) => {
          arrowsContainer.appendChild(arrow.cloneNode(true));
        });

        // Adicionar eventos aos novos botões
        const newPrev = arrowsContainer.querySelector(".splide__arrow--prev");
        const newNext = arrowsContainer.querySelector(".splide__arrow--next");
