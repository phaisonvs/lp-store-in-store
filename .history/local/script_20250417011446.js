...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupVideoCarouselControls = () => {
    ...
    // Garantir que todos os vídeos estão visíveis
    setupVideoVisibility(carousel);
  ...
  /**
   * @description Configura a visibilidade dos vídeos - versão simplificada
   * @param {HTMLElement} container - Contêiner dos vídeos
   */
  const setupVideoVisibility = (container) => {
    const videoFrames = container.querySelectorAll("iframe");

    // Observer para tratar vídeos quando entram no viewport
