document.addEventListener("DOMContentLoaded", function () {
  ...
  // Configurações do efeito parallax
  const parallaxSettings = {
    speed: 0.5, // Velocidade do efeito (menor = mais sutil)
    scale: 1.1, // Escala inicial do background (> 1 para zoom)
    maxShift: 50, // Movimento máximo em pixels
    mouseInfluence: 0.1, // Influência do mouse no efeito
  };
  ...
  function setupParallaxEffect() {
    ...
    updateParallaxBackground(parallaxElement, 0, 0, 0);
    ...
    window.addEventListener("scroll", () => {
      ...
      if (elementVisible) {
        const scrollPercentage =
          (window.innerHeight - elementRect.top) /
    ...
    let currentScale = parallaxSettings.scale;
    const minScale = parallaxSettings.scale;
    const maxScale = parallaxSettings.scale + 0.05;
    const scaleSpeed = 0.01;
    let scaleDirection = 1;

    function animateScale() {
      // Alterar a escala suavemente
      currentScale += scaleSpeed * scaleDirection * 0.05;

      // Inverter direção quando chegar aos limites
      if (currentScale >= maxScale) scaleDirection = -1;
      if (currentScale <= minScale) scaleDirection = 1;

      // Aplicar a escala
      if (parallaxElement) {
        parallaxElement.style.backgroundSize = `${Math.floor(
          currentScale * 110
        )}%`;
      }
  ...
  // Função para atualizar o background com efeito parallax
  function updateParallaxBackground(
    ...
    // Calcular o deslocamento vertical baseado no scroll
    const verticalShift = scrollPosition * parallaxSettings.speed;

    // Aplicar uma transformação baseada na posição do scroll e do mouse
    let xShift = 0;
    let yShift = -verticalShift;

    // Se mouse estiver presente, adicionar sua influência
    if (mouse) {
      xShift += mouse.x * parallaxSettings.mouseInfluence;
      yShift += mouse.y * parallaxSettings.mouseInfluence;
    }

    // Limitar o deslocamento máximo
    xShift = Math.max(
      -parallaxSettings.maxShift,
      Math.min(parallaxSettings.maxShift, xShift)
    );
    yShift = Math.max(
      -parallaxSettings.maxShift,
      Math.min(parallaxSettings.maxShift, yShift)
    );

    // Aplicar o efeito
    element.style.backgroundPosition = `calc(50% + ${xShift}px) calc(${yShift}px)`;
  }
