  ...
  function initVideoSlider() {
    ...
    function updateControls() {
      ...
      if (paginationDots.length > 0) {
        const currentPage = Math.floor(state.currentIndex / state.perPage);
    ...
    function dragEnd() {
      ...
      // Calcular índice mais próximo com base na posição atual
      ...
      if (Math.abs(movedBy) > 20) {
        ...
        const moveBy = Math.min(
          Math.max(1, Math.abs(Math.round(movedBy / state.slideWidth))),
          config.perMove
        );
        goToSlide(state.currentIndex + direction * moveBy);
      } else {
