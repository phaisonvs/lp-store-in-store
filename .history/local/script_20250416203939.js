document.addEventListener("DOMContentLoaded", function () {
  ...
  // Registrar o plugin ScrollTrigger para GSAP
  try {
    gsap.registerPlugin(ScrollTrigger);
    setupParallaxEffects();
  } catch (error) {
    console.error("GSAP ou ScrollTrigger não estão disponíveis:", error);
  }

  // Configurar efeitos parallax
  function setupParallaxEffects() {
    ...
    if (parallaxSection) {
      ...
      gsap.to(parallaxSection, {
        backgroundSize: "120%", // Aumento até 120%
        ease: "none",
        scrollTrigger: {
          trigger: parallaxSection,
          start: "top bottom",
          end: "bottom top",
          scrub: true, // Suaviza o efeito durante o scroll
        },
      });
    }
  }
