...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const state = {
    guideShopCarousel: {
      ...
      autoplayEnabled: false, // Desativado por padrão
    },
    animations: {
      observer: null,
      initialized: true, // Garantir que animações sejam consideradas já inicializadas
      sections: [
        ".big-numbers",
        ".section-the-news",
  ...
  /**
   * @description Configura todos os elementos para serem visíveis imediatamente sem animação
   */
  const setupScrollAnimations = () => {
    // Desativa animações e torna elementos visíveis imediatamente
    const animatedElements = document.querySelectorAll(
      ".animate-on-scroll, .container-prova-social-1, .section-the-news, .nossas-guides-container, .carousel, .svg-suf-desk, .svg-suf-mobile, .faca-parte, .big-numbers ul li, .card-prova-social, .section-franquias"
    );
  ...
  /**
   * @description Configura o lazy loading de vídeos usando IntersectionObserver - otimizado para performance
   * Modificado para carregar todos os vídeos imediatamente
   */
  const setupLazyVideos = () => {
    ...
    lazyVideos.forEach((videoContainer) => {
      ...
      if (iframe) {
        // Verificar se o src já está definido ou se existe um data-src
        ...
        // Garantir que o container do vídeo esteja visível
        videoContainer.style.opacity = "1";
        videoContainer.style.visibility = "visible";
      }
    });

    // Desativar qualquer observer de vídeo existente
  ...
  function initVideoSlider() {
    ...
    pagination.className = "video-pagination";
    sliderContainer.appendChild(pagination);

    // Criar dots para paginação
    for (let i = 0; i < totalGroups; i++) {
      const dot = document.createElement("button");
      dot.className = i === 0 ? "video-dot active" : "video-dot";
      dot.setAttribute("aria-label", `Ir para grupo de vídeos ${i + 1}`);
    ...
    // Definir o foco no container do slider para controle por teclado
    sliderContainer.setAttribute("tabindex", "0");
    sliderContainer.setAttribute("role", "region");
    sliderContainer.setAttribute("aria-label", "Carrossel de vídeos");
    ...
    if (prevButton) {
      ...
      prevButton.addEventListener("mousedown", handleButtonPress);
      prevButton.addEventListener("mouseup", handleButtonRelease);
      prevButton.addEventListener("mouseleave", handleButtonRelease);
    }

    if (nextButton) {
      nextButton.addEventListener("click", handleNextClick);
      nextButton.addEventListener("mousedown", handleButtonPress);
      nextButton.addEventListener("mouseup", handleButtonRelease);
    ...
    // Inicializar o estado dos botões de navegação
    ...
    function drag(event) {
      ...
      // Resistência ao arrastar além dos limites
      if (currentTranslate > 0) {
        currentTranslate = currentTranslate * 0.3;
      } else if (currentTranslate < getMaxTranslate()) {
        const overDrag = getMaxTranslate() - currentTranslate;
        currentTranslate = getMaxTranslate() - overDrag * 0.3;
      }
    }
    ...
    function endDrag() {
      ...
      // Determinar se deve avançar ou retroceder com base no movimento
      if (movedBy < -100 && currentTranslate < prevTranslate) {
        navigateToGroup(Math.min(currentGroup + 1, totalGroups - 1));
      } else if (movedBy > 100 && currentTranslate > prevTranslate) {
        navigateToGroup(Math.max(currentGroup - 1, 0));
      } else {
        // Voltar para o grupo atual se o movimento não for suficiente
        navigateToGroup(currentGroup);
      }
    ...
    function getMaxTranslate() {
      return -((totalSlides - slidesPerView) * (slideWidth + slideGap));
    }
    ...
    function navigateToGroup(groupIndex) {
      ...
      // Calcular a nova posição de translate
      const newTranslate = -(
        groupIndex *
        slidesPerView *
        (slideWidth + slideGap)
      ...
      // Ajustar para que não vá além do limite
      currentTranslate = Math.max(newTranslate, getMaxTranslate());
    ...
    function updateNavigationState() {
      if (prevButton) {
        prevButton.setAttribute(
          "aria-disabled",
          currentGroup === 0 ? "true" : "false"
        );
        prevButton.style.opacity = currentGroup === 0 ? "0.5" : "1";
        prevButton.style.pointerEvents = currentGroup === 0 ? "none" : "auto";
      }
  ...
  // Inicializar o slider de vídeos
  initVideoSlider();

  // Tornar todos os elementos imediatamente visíveis
  const allElementsToShow = [
    ...document.querySelectorAll(
      ".animate-on-scroll, .container-prova-social-1, .section-the-news, .nossas-guides-container, .carousel, .card-prova-social, .section-franquias, .section-content, .section-perguntas-frequentes, .section-blog-posts, .clientes-dizem-section, .section-banner-form"
    ),
  ];
  ...
  // Inicializar componentes necessários sem animações
  setupScrollAnimations();
  setupLazyVideos();

  // Inicializar carrossel com visibilidade imediata
  ...
  if (carousel) {
    ...
    // Adicionar setas de navegação sem animação
