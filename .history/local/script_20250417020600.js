...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupGuideShopCarouselEventListeners = (carousel, track) => {
    ...
    console.log("Botões do carrossel de Guide Shops encontrados:", {
  ...
  const setupVideoCarouselControls = () => {
    ...
    console.log("Estado inicial do carrossel de vídeos:", {
    ...
    const videos = track.querySelectorAll(".card-prova-social");
    console.log("Vídeos encontrados:", videos.length);

    if (videos.length === 0) return;

    // Configurar o estado do carrossel de vídeos
    const videoCarouselState = {
      isDragging: false,
      startPosition: 0,
      currentTranslate: 0,
      prevTranslate: 0,
    ...
    const scrollAmount = cardWidth + cardMarginRight + cardMarginLeft;
    console.log("Largura exata para navegação:", scrollAmount);

    // Número máximo de slides visíveis
    const maxIndex = videos.length - 1;

    // Função para navegar por índice (como no carrossel de franquias)
    const navigateToIndex = (index) => {
      // Garantir que o índice esteja dentro dos limites
      if (index < 0) index = 0;
      if (index > maxIndex) index = maxIndex;

      videoCarouselState.currentIndex = index;
      videoCarouselState.currentTranslate = -index * scrollAmount;

      // Cancelar qualquer animação existente
      if (videoCarouselState.animationID) {
        cancelAnimationFrame(videoCarouselState.animationID);
      }

      // Usar a mesma transição suave do carrossel de franquias
      requestAnimationFrame(() => {
        track.style.transition = "transform 0.5s var(--ease-out-smooth)";
        track.style.transform = `translateX(${videoCarouselState.currentTranslate}px)`;
      });
    };

    // Eventos de navegação para as setas (igual ao carrossel de franquias)
    prevButton.addEventListener("click", () => {
      console.log("Navegando para o vídeo anterior");
      navigateToIndex(videoCarouselState.currentIndex - 1);
    });
    ...
    // Eventos de arrasto (drag) como no carrossel de franquias
    const pointerStart = (e) => {
      if (videoCarouselState.isDragging) return;

      videoCarouselState.isDragging = true;
      videoCarouselState.startPosition =
        e.pageX || (e.touches && e.touches[0].pageX) || 0;
      videoCarouselState.prevTranslate = videoCarouselState.currentTranslate;

      // Cancelar a animação durante o arrasto
      if (videoCarouselState.animationID) {
        cancelAnimationFrame(videoCarouselState.animationID);
      }

      // Mudar aparência durante arrasto
    ...
    const pointerMove = (e) => {
      ...
      const currentPosition = e.pageX || (e.touches && e.touches[0].pageX) || 0;
      const diff = currentPosition - videoCarouselState.startPosition;
      ...
      // Calcular nova posição com melhor precisão
      videoCarouselState.currentTranslate =
        videoCarouselState.prevTranslate + dampedDiff;
    ...
    const pointerEnd = () => {
      if (!videoCarouselState.isDragging) return;
      videoCarouselState.isDragging = false;
      carousel.classList.remove("is-dragging");

      // Determinar o índice mais próximo com base na posição atual
      const movedDistance =
        videoCarouselState.currentTranslate - videoCarouselState.prevTranslate;

      // Se moveu o suficiente, avance/retroceda para o próximo slide
      if (Math.abs(movedDistance) > scrollAmount / 3) {
        if (movedDistance > 0) {
        ...
        // Voltar para o slide atual se não moveu o suficiente
        navigateToIndex(videoCarouselState.currentIndex);
      }
    };
    ...
    window.addEventListener(
      "touchmove",
      (e) => {
        if (videoCarouselState.isDragging) {
          pointerMove({ pageX: e.touches[0].pageX });
        }
      },
      { passive: true }
    ...
    // Depois que a transição terminar, remover a transição para evitar atrasos no arrasto
