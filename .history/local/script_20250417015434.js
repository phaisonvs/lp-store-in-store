...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setupVideoCarouselControls = () => {
    ...
    // Detecção de fim do scroll para atualizar o índice atual
    carousel.addEventListener(
      "scroll",
      () => {
        // Usar requestAnimationFrame para limitar chamadas durante scroll
        requestAnimationFrame(() => {
          const scrollPos = carousel.scrollLeft;
          // Calcular o índice baseado na posição de scroll
          currentIndex = Math.round(scrollPos / scrollAmount);
        });
      },
      { passive: true }
    );
