...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const state = {
    ...
    parallax: {
      initialZoom: 1.05, // Zoom inicial
      minZoom: 1.0, // Zoom mínimo
      maxZoom: 1.1, // Zoom máximo
      animationId: null, // Para requestAnimationFrame
    },
    isInitialized: false,
  };

  /**
   * @description Inicializa o parallax com zoom suave
   * @returns {void}
   */
  const setupParallax = () => {
    ...
    // Configurar o efeito de parallax com zoom
    const handleScroll = () => {
      ...
      // Parallax - Calcular o deslocamento do fundo com base no scroll
      const speed = 0.15; // Velocidade do efeito (quanto menor, mais suave)
      const yPos = -scrollPosition * speed;

      // Calcular o progresso do scroll em relação à seção (0 a 1)
      ...
      // Zoom - Calcular o zoom baseado na posição do scroll
      // Inicialmente aumenta (quando entra na tela), depois diminui (ao sair da tela)
      const zoomCurve = Math.sin(scrollProgress * Math.PI);
      const zoomRange = state.parallax.maxZoom - state.parallax.minZoom;
      const currentZoom = state.parallax.minZoom + zoomRange * zoomCurve;

      // Aplicar o efeito parallax e zoom
      section.style.backgroundPosition = `center ${yPos}px`;
      section.style.backgroundSize = `${currentZoom * 100}%`;
    };
  ...
  const init = () => {
    ...
    // Aplicar o efeito de parallax com zoom
    setupParallax();
